...
-- Refactor public.create_public_booking to revert to a clean manual dispatch workflow
...
begin
  ...
  -- 1. Determine status: manual dispatch starts with pending (or pending_payment if not cash)
  v_status := case when nullif(payload->>'payment','') = 'Cash' then 'pending' else 'pending_payment' end;
  ...
  insert into public.bookings (
    ...
    -- No auto-assigned driver details in Phase 1 (Reverted to clean manual dispatch)
    null,
    null,
    null,
    null
