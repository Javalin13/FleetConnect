...
-- (CORRECTED per Lux 8d5d099 BLOCKER #1 — function creation order)
--
-- Mission: 2026-08-29-fleetconnect-operational-recovery
-- Round: r056 Phase F (batch 1 corrections)
-- Authority: mailbox-audit.md (r053 Phase 7) + Lux 68f35b6 §4 (Phase F active) + Lux 8d5d099 BLOCKER #1
--
-- BLOCKER #1 FIX: this migration now creates `authorize_dispatch_mailbox()`
-- in section 1 (BEFORE any RLS policy references it). Section ordering is:
--   1. authorize_dispatch_mailbox() RPC       — created FIRST so policies can resolve
--   2. log_dispatch_mailbox_action() RPC      — also created early (policies depend on its sibling)
...
-- =========================================================
-- 1. authorize_dispatch_mailbox() — CREATED FIRST (BLOCKER #1 FIX)
-- =========================================================
...
-- =========================================================
-- 8. RLS policies — now reference existing authorize_dispatch_mailbox() (section 1)
-- =========================================================
