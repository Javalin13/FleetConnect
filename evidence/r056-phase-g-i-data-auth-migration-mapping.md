...
3. **For each operator / partner / driver: CREATE the user in the
   new project FIRST (this is a precondition — a recovery email
   cannot be sent to an account that does not yet exist).**
   Founder-authenticated create-user flow (Option C1 only — Option
   C2 was REMOVED per Lux 39ca1a0 §5):
   - **Option C1 (canonical, supported):** Dashboard →
     Authentication → Users → "Add user" → "Create new user" with
     email + auto-confirm checked. The new `auth.users.id` is
     returned by the Dashboard and recorded in the mapping
     document.
   - ~~Option C2 (REMOVED per Lux 39ca1a0 §5): direct
     `INSERT INTO auth.users ...` / `INSERT INTO auth.identities
     ...` via SQL Editor — this directly manipulates Supabase-
     managed Auth internals and reintroduces the exact class of
     unsupported raw-auth mutation Lux previously rejected. It is
     NOT acceptable as a canonical or fallback re-onboarding path,
     even when the Founder is authenticated in Dashboard.~~
   - *(Future option, not yet adopted)* If a supported
     administrative auth API is later verified and reviewed, it
     MAY be added as another canonical option in a future round.
     Until then, Dashboard is the only supported path.
4. **Then send invite / recovery / reset** for the newly created
   account:
   - For operator / partner / driver: Dashboard → Authentication
     → Users → select user → "Send recovery email" — so the user
     sets their own password
   - For customer: send a "your account is on the new platform,
     please re-set your password" email
5. **Capture the resulting NEW `auth.users.id`** (this is the
   source of truth for the old → new mapping):
   - The new `id` is generated by Supabase during create
   - It MUST be recorded at create time, not assumed later
   - **Do NOT assume legacy `auth.users.id` portability.** The
     cross-project portability of `auth.users.id` is not a
     documented Supabase feature; the legacy IDs belong to the
     legacy project's `auth` schema. The target-created ID is
     authoritative unless a separately supported migration method
     explicitly preserves IDs (no such method is currently adopted).
6. **Apply the deterministic old → new `user_id` mapping** to
   application rows (in the same authenticated Dashboard/CLI
   session):
   - `UPDATE public.customers SET user_id = '<NEW_id>' WHERE
     legacy_user_id = '<OLD_id>';`
   - `UPDATE public.partners SET user_id = '<NEW_id>' WHERE
     legacy_user_id = '<OLD_id>';`
   - `UPDATE public.drivers SET user_id = '<NEW_id>' WHERE
     legacy_user_id = '<OLD_id>';`
   - This is the point where Option B (legacy_user_id audit
     column) pays off.
7. **Do NOT assume old UUIDs survive** in the new project's
   `auth.users`. The old IDs belong to the legacy project's
   `auth` schema; cross-project `auth.users.id` portability is
   not a documented Supabase feature.
8. **Do NOT raw-import auth internals** unless a separately
   reviewed current Supabase-supported migration method is
   proven (per Lux f0626bd §3 + Lux 7aac5aa §8 + Lux 39ca1a0 §5).
...
+--------------------+             +--------------------+
| id = aaaa-...      |   CREATE    | id = zzzz-...      |
| email = a@b.com   | --new-----> | email = a@b.com    |
| hash = bcrypt(...) |  (Option    | hash = bcrypt(...) |
+--------------------+   C1/C2)    +--------------------+
         |                                  |
         | (audit, preserved)               | (FK after re-onboard)
         v                                  v
